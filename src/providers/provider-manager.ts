...
import { ensureString } from "../utils/safety-guards.js"
// Import providers
import { AnthropicProvider } from "./anthropic-provider.js"
import { CohereProvider } from "./cohere-provider.js"
// Phase 5: Provider Integration imports
...
import { VsCodeLmOrchestrationHandler } from "./vscode-lm-orchestration-handler.js"
...
export class ProviderManager extends EventEmitter {
	...
	constructor(logger: ILogger, _configManager: ConfigManager, config: ProviderManagerConfig) {
		super()
		this.logger = logger
		this.config = config
	...
	private async selectOptimalProvider(request: LLMRequest): Promise<ILLMProvider | null> {
		...
		for (const provider of this.providers.values()) {
			...
			try {
				...
				this.logger.warn(`Failed to estimate cost for ${ensureString(provider.name || "unknown", "unknown")}`, error)
			}
		}
	 ...
	 */
	private async handleRequestError(error: unknown, request: LLMRequest, failedProvider: ILLMProvider): Promise<LLMResponse> {
	 ...
	 */
	private async getFallbackProvider(error: unknown, failedProvider: ILLMProvider): Promise<ILLMProvider | null> {
	 ...
	 */
	private updateProviderMetrics(provider: LLMProvider, metrics: { success: boolean; latency: number; cost: number }): void {
