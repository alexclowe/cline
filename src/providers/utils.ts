...
import { FallbackStrategy, LLMProvider, LLMProviderConfig } from "./types.js"
 ...
 */
function loadProviderConfigs(configs: Record<LLMProvider, LLMProviderConfig>): Record<LLMProvider, LLMProviderConfig> {
	...
	for (const [provider, config] of Object.entries(loaded)) {
		...
		if (process.env[`${envPrefix}TEMPERATURE`]) {
			config.temperature = parseFloat(ensureString(process.env[`${envPrefix}TEMPERATURE`], "0.7"))
		}
		if (process.env[`${envPrefix}MAX_TOKENS`]) {
			config.maxTokens = parseInt(ensureString(process.env[`${envPrefix}MAX_TOKENS`], "4096"), 10)
		}
		if (process.env[`${envPrefix}API_URL`]) {
			config.apiUrl = ensureString(process.env[`${envPrefix}API_URL`])
		}
	}
