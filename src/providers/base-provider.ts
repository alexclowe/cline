...
import { CircuitBreaker, circuitBreaker } from "../utils/helpers"
import {
	CostEstimate,
	HealthCheckResult,
	ILLMProvider,
...
export abstract class BaseProvider extends EventEmitter implements ILLMProvider {
	...
	constructor(options: BaseProviderOptions) {
		...
		// Circuit breaker will be initialized in doInitialize() after name is available
		this.circuitBreaker = null as any // Temporary assignment
	...
	async initialize(): Promise<void> {
		// Initialize circuit breaker now that name is available
	...
	getStatus(): ProviderStatus {
		...
		const _errorRate = this.requestCount > 0 ? this.errorCount / this.requestCount : 0
	...
	async getUsage(period: UsagePeriod = "day"): Promise<UsageStats> {
		...
		return {
			...
			modelBreakdown: {} as Record<string, any>, // Would need to track per model
		}
	}
	 ...
	 */
	protected trackRequest(_request: LLMRequest, response: LLMResponse, latency: number): void {
		this.requestCount++
