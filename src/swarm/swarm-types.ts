 ...
 * Based on the working Claude Flow reference implementation
 */
...
export interface SwarmResults {
	...
	// Success metrics
	objectivesMet: TaskId[]
	objectivesFailed: TaskId[]
...
export interface SwarmTask {
	...
	context: {
		previousResults: SwarmTaskResult[]
		relatedTasks: SwarmTask[]
		dependencies: TaskId[]
	}
...
export interface SwarmConfiguration {
	...
	failureHandling: "abort" | "retry" | "reassign"
}
...
// Enhanced memory entry for swarm operations
export interface SwarmMemoryEntry {
	id: string
	type: "task" | "result" | "agent" | "swarm_definition" | "event"
	content: string
	metadata: {
		taskId?: string
		objectiveId?: string
...
export class TaskExecutionError extends SwarmError {
	constructor(
		...
		super(message, "TASK_EXECUTION_ERROR", undefined, taskId, agentId)
