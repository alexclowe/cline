...
import { AgentState, SWARM_CONSTANTS, TaskDefinition } from "./types.js"
...
export class TaskExecutor extends EventEmitter {
	...
	constructor(config: Partial<ExecutionConfig> = {}) {
		...
		this.logger = new Logger(
			{
				level: (this.config.logLevel as "error" | "info" | "debug" | "warn") || "info",
				format: "text",
				destination: "console",
			},
	...
	async executeTask(task: TaskDefinition, agent: AgentState, options: Partial<ExecutionConfig> = {}): Promise<ExecutionResult> {
	...
	private buildClaudeCommand(task: TaskDefinition, agent: AgentState, options: ClaudeExecutionOptions): ClaudeCommand {
		const args: string[] = []
		let input = ""
	...
	private async createExecutionContext(task: TaskDefinition, agent: AgentState): Promise<ExecutionContext> {
...
class ResourceMonitor extends EventEmitter {
	...
	private collectCurrentUsage(_sessionId: string): ResourceUsage {
