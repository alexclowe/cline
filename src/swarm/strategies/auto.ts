...
import type { AgentState, SwarmObjective, TaskDefinition, TaskId, TaskPriority, TaskType } from "../types.js"
import type { AgentAllocation, DecompositionResult, TaskBatch, TaskPattern } from "./base.js"
...
export class AutoStrategy extends BaseStrategy {
	...
	private determineOptimalTaskStructure(_patterns: TaskPattern[], taskTypes: string[], complexity: number) {
		return {
	...
	private createTaskBatches(tasks: TaskDefinition[], _dependencies: Map<string, string[]>): TaskBatch[] {
	...
	private async createPredictiveSchedule(tasks: TaskDefinition[], _agents: AgentState[]): Promise<PredictiveSchedule> {
		...
		agents.forEach((agent) => {
			...
			if (suitableTasks.length > 0) {
				allocations.push({
					...
					capabilities: Object.keys(agent.capabilities).filter((cap) => (agent.capabilities as any)[cap] === true),
				})
			}
		})
