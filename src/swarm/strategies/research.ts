...
import { SWARM_CONSTANTS, SwarmConfig, SwarmObjective, TaskDefinition, TaskId, TaskPriority, TaskType } from "../types.js"
...
export class ResearchStrategy extends BaseStrategy {
	...
	constructor(config: Partial<SwarmConfig> = {}) {
		...
		this.logger = new Logger({ level: "info", format: "text", destination: "console" }, { component: "ResearchStrategy" })
...
Ensure the report is well-structured and actionable.`,
	...
	private async executeGenericResearchTask(task: TaskDefinition, _agent: any): Promise<any> {
	...
	private extractTimeframe(_description: string): any {
	...
	private extractSourceTypes(_description: string): string[] {
	...
	private async executeRateLimitedSearch(query: ResearchQuery, _agent: any): Promise<ResearchResult[]> {
	...
	private createParallelExtractionTasks(task: TaskDefinition, _agent: any): Promise<any>[] {
	...
	private releasePooledConnection(_connection: any): void {
		this.connectionPool.active--
	...
	private updateResearchMetrics(_taskType: string, duration: number): void {
	...
	private createTaskBatches(tasks: TaskDefinition[], _dependencies: Map<string, string[]>): any[] {
		const batches: any[] = []
	...
	override getMetrics() {
		...
		return {
			...
			cacheHitRate:
				this.researchMetrics.cacheHits / (this.researchMetrics.cacheHits + this.researchMetrics.cacheMisses || 1),
	...
	async refineResearchScope(objective: SwarmObjective, intermediateResults: any[]): Promise<SwarmObjective> {
		...
		if (intermediateResults.length > 0) {
			const avgCredibility =
				intermediateResults.map((r) => r.credibilityScore || 0.5).reduce((a, b) => a + b, 0) / intermediateResults.length
