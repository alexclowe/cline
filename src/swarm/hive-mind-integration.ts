...
import { AgentCapabilities, SwarmAgent } from "./types"
...
export class HiveMindIntegration extends EventEmitter {
	 ...
	 */
	async createSession(swarmId: string, _orchestrator: AdvancedSwarmOrchestrator): Promise<string> {
		...
		try {
			...
			if (knowledgeEntries && Array.isArray(knowledgeEntries)) {
				for (const entry of knowledgeEntries) {
					if (entry.type === "metadata" && entry.content) {
	...
	private async loadCollectiveIntelligence(): Promise<void> {
		try {
			...
			if (intelligenceEntries && Array.isArray(intelligenceEntries)) {
	...
	private async shareKnowledgeWithAgent(session: HiveMindSession, agentId: string, agent: SwarmAgent): Promise<void> {
	...
	private getRelevantKnowledge(session: HiveMindSession, capabilities: AgentCapabilities): any[] {
		const relevantItems: any[] = []

		// Extract capability names from AgentCapabilities interface
		const capabilityNames = this.extractCapabilityNames(capabilities)

		// Filter facts by capabilities
		for (const fact of session.knowledgeBase.facts.values()) {
			if (capabilityNames.some((cap) => fact.category.includes(cap))) {
		...
		for (const procedure of session.knowledgeBase.procedures.values()) {
			if (capabilityNames.some((cap) => procedure.contexts.includes(cap))) {
	...
	private extractCapabilityNames(capabilities: AgentCapabilities): string[] {
		const capabilityNames: string[] = []

		// Extract boolean capabilities
		if (capabilities.codeGeneration) {
			capabilityNames.push("codeGeneration")
		}
		if (capabilities.codeReview) {
			capabilityNames.push("codeReview")
		}
		if (capabilities.testing) {
			capabilityNames.push("testing")
		}
		if (capabilities.documentation) {
			capabilityNames.push("documentation")
		}
		if (capabilities.research) {
			capabilityNames.push("research")
		}
		if (capabilities.analysis) {
			capabilityNames.push("analysis")
		}
		if (capabilities.webSearch) {
			capabilityNames.push("webSearch")
		}
		if (capabilities.apiIntegration) {
			capabilityNames.push("apiIntegration")
		}
		if (capabilities.fileSystem) {
			capabilityNames.push("fileSystem")
		}
		if (capabilities.terminalAccess) {
			capabilityNames.push("terminalAccess")
		}

		// Extract array capabilities
		capabilityNames.push(...capabilities.languages)
		capabilityNames.push(...capabilities.frameworks)
		capabilityNames.push(...capabilities.domains)
		capabilityNames.push(...capabilities.tools)
	...
	private queryFacts(session: HiveMindSession, query: any): Fact[] {
		...
		for (const fact of session.knowledgeBase.facts.values()) {
			...
			if (
				query.keywords &&
				!query.keywords.some((keyword: string) => fact.statement.toLowerCase().includes(keyword.toLowerCase()))
			) {
				matches = false
			}
	...
	private async syncSessionKnowledge(_session: HiveMindSession): Promise<void> {
