...
import TaskExecutor, { ClaudeExecutionOptions, ExecutionContext, ExecutionResult } from "./executor.js"
import { AgentCapabilities, AgentState, TaskDefinition } from "./types"
...
export class ClaudeCodeInterface extends EventEmitter {
	...
	async spawnAgent(options: ClaudeSpawnOptions): Promise<string> {
		...
		try {
			...
			// Spawn process
			const claudeProcess = spawn(command.executable, command.args, {
	...
	getInterfaceMetrics(): {
		...
		const avgResponseTime =
			agents.length > 0 ? agents.reduce((sum, a) => sum + a.metrics.averageResponseTime, 0) / agents.length : 0
