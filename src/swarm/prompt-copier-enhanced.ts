...
export class EnhancedPromptCopier extends PromptCopier {
	...
	// Override the copy method to use our enhanced parallel implementation
	override async copy(): Promise<CopyResult> {
		const startTime = Date.now()

		try {
			// Phase 1: Discovery
			await (this as any).discoverFiles()

			if ((this as any).fileQueue.length === 0) {
				return {
					success: true,
					totalFiles: 0,
					copiedFiles: 0,
			...
			if (!(this as any).options.dryRun) {
				await (this as any).ensureDestinationDirectories()
			}

			// Phase 3: Copy files with enhanced parallel processing
			if ((this as any).options.parallel) {
				await this.copyFilesParallelEnhanced()
			} else {
				await (this as any).copyFilesSequential()
			}
			...
			const result: any = {
				success: (this as any).errors.length === 0,
				totalFiles: (this as any).fileQueue.length,
				copiedFiles: (this as any).copiedFiles.size,
				failedFiles: (this as any).errors.length,
				skippedFiles: (this as any).fileQueue.length - (this as any).copiedFiles.size - (this as any).errors.length,
				errors: (this as any).errors,
				duration,
			}

			if ((this as any).backupMap.size > 0) {
				result.backupLocation = await (this as any).createBackupManifest()
			}

			return result
		} catch (error) {
			if (!(this as any).options.dryRun) {
				await (this as any).rollback()
			}
			throw error
		}
	}
	...
	private processChunkResults(_chunk: FileInfo[], results: any[]): void {
	...
	private handleWorkerResult(result: any, workerId: number, _pool: WorkerPool): void {
...
// Re-export the basic copyPrompts function
