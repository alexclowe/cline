...
import type { MCPContext, MCPTool } from "../mcp/index"
import { createRuvSwarmTools } from "../mcp/ruv-swarm-tools"
import { generateId } from "../utils/helpers"
import type { ILogger, LoggingConfig } from "../utils/types"
import type { AdvancedSwarmOrchestrator } from "./advanced-orchestrator"
...
export class MCPIntegrationWrapper extends EventEmitter {
	...
	constructor(config: Partial<MCPIntegrationConfig> = {}, logger?: ILogger) {
		super()
	 ...
	 */
	async executeTool(toolName: string, input: any, context: MCPExecutionContext): Promise<MCPToolExecutionResult> {
	 ...
	 */
	getAvailableTools(options: { category?: string; capability?: string; agent?: SwarmAgent } = {}): MCPTool[] {
	...
	private isNonRetryableError(error: Error): boolean {
		const nonRetryablePatterns = [/not found/i, /invalid input/i, /permission denied/i, /unauthorized/i, /forbidden/i]
	...
	private hasPermission(tool: MCPTool, agent: SwarmAgent): boolean {
		...
		// Convert capabilities to array if it's not already
		const capabilities = Array.isArray(agent.capabilities) ? agent.capabilities : agent.capabilities.tools || []

		return capabilities.some((capability: string) => toolPermissions.includes(capability))
	}
	...
	private createDefaultLogger(): ILogger {
		return {
			...
			warn: () => {},
			error: () => {},
			configure: async (_config: LoggingConfig) => {},
		}
	}
}
