...
import { createProgressBar, formatDuration, PromptConfigManager, PromptPathResolver, PromptValidator } from "./prompt-utils.js"
			...
			if (stats.isFile()) {
				...
				const scanDir = async (dir: string) => {
					...
					for (const entry of entries) {
						...
						if (
							entry.isFile() &&
							(entry.name.endsWith(".md") || entry.name.endsWith(".txt") || entry.name.endsWith(".prompt"))
						) {
							files.push(fullPath)
