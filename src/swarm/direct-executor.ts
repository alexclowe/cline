...
import type { ILogger } from "../utils/types"
...
export class DirectTaskExecutor {
	...
	private async executeTaskByType(task: TaskDefinition, agent: AgentState, targetDir?: string): Promise<any> {
		...
		switch (agent.type) {
			...
			case "coder":
				if (!targetDir) {
					throw new Error("Target directory is required for coder tasks")
				}
				if (isRestAPI) {
	...
	private async createCalculator(targetDir: string, _task: TaskDefinition): Promise<any> {
...
`,
	...
	private async executeReviewTask(task: TaskDefinition, targetDir?: string): Promise<any> {
		...
		const review = {
			...
			findings: ["Code follows best practices", "Proper error handling implemented", "Documentation is comprehensive"],
			recommendations: ["Consider adding more unit tests", "Optimize database queries", "Add input validation"],
		}
...
`
	...
	private async executeResearchTask(task: TaskDefinition, targetDir?: string): Promise<any> {
		...
		const research = {
			taskName: task.name,
			findings: ["Best practices identified", "Similar implementations analyzed", "Performance benchmarks reviewed"],
			recommendations: ["Use established patterns", "Follow industry standards", "Implement security best practices"],
		}
	...
	private async executeCoordinationTask(task: TaskDefinition, _targetDir?: string): Promise<any> {
	...
	private async executeGenericTask(task: TaskDefinition, _targetDir?: string): Promise<any> {
	...
	private generateRestAPIServer(_task: TaskDefinition): string {
...
`
	...
	private generateTodoApp(_task: TaskDefinition): string {
...
`
	...
	private generateChatServer(_task: TaskDefinition): string {
		return `const express = require('express');
...
`
	...
	private generateAuthServer(_task: TaskDefinition): string {
		return `const express = require('express');
