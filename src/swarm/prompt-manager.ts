...
export class PromptManager extends EventEmitter {
		...
		if (this.options.autoDiscovery) {
			...
			if (discovered.length > 0) {
				...
				const uniqueDirs = Array.from(
					new Set([...config.sourceDirectories, ...discovered.map((dir) => path.relative(this.options.basePath, dir))]),
				)
	...
	private async performIncrementalSync(source: string, destination: string, options: SyncOptions): Promise<CopyResult> {
