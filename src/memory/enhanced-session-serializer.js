...
import { createSessionSerializer, DeserializationError, SerializationError } from "./advanced-serializer.js"
...
export class SessionSerializer {
	 ...
	 */
	_postprocessSessionData(sessionData, _options = {}) {
		if (!sessionData || typeof sessionData !== "object") {
			return sessionData
		}
	...
	_preprocessAgent(agent) {
		...
		if (processed.last_active) {
			processed.last_active = new Date(processed.last_active)
		}
	...
	_preprocessTask(task) {
		...
		if (processed.started_at) {
			processed.started_at = new Date(processed.started_at)
		}
		if (processed.completed_at) {
			processed.completed_at = new Date(processed.completed_at)
		}
