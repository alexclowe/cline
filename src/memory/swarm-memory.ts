...
export class SwarmMemoryManager extends EventEmitter {
	...
	async remember(
		...
		await this.baseMemory.store({
			id: `${this.config.namespace}:entry:${entryId}`,
			agentId,
			content: JSON.stringify(entry),
			type: "metadata",
			timestamp: new Date(),
			tags: [type, agentId, entry.metadata.shareLevel || "team"],
			metadata: {
				type: "swarm-memory",
				agentId,
				entryType: type,
	...
	async createKnowledgeBase(name: string, description: string, domain: string, expertise: string[]): Promise<string> {
	...
	async searchKnowledge(query: string, domain?: string, expertise?: string[]): Promise<SwarmMemoryEntry[]> {
