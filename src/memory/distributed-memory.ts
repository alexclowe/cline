...
import type { AccessLevel, AgentId, ConsistencyLevel, MemoryEntry, MemoryPartition, MemoryType } from "../swarm/types.js"
...
export class DistributedMemorySystem extends EventEmitter {
	 ...
	 */
	async queryByType(type: MemoryType, namespace?: string): Promise<MemoryEntry[]> {
		return this.query({
			type,
			namespace,
		})
	...
	private async mergeValues(oldValue: any, newValue: any, _partition: MemoryPartition): Promise<any> {
	...
	private checkAccess(_entry: MemoryEntry, _operation: "read" | "write" | "delete"): boolean {
	...
	private updateCache(_entryId: string, entry: MemoryEntry): void {
	...
	private async replicateEntry(_entry: MemoryEntry): Promise<void> {
		// Implementation for replication to other nodes
	}

	private async syncPartitionCreation(_partition: MemoryPartition): Promise<void> {
		// Implementation for syncing partition creation
	}

	private async syncPartitionDeletion(_partitionId: string): Promise<void> {
		// Implementation for syncing partition deletion
	}

	private async syncEntryUpdate(_entry: MemoryEntry): Promise<void> {
		// Implementation for syncing entry updates
	}

	private async syncEntryDeletion(_entryId: string): Promise<void> {
		// Implementation for syncing entry deletion
	}

	private async retrieveFromRemote(_key: string, _options: any): Promise<MemoryEntry | null> {
	...
	private async executeSyncOperation(_operation: SyncOperation): Promise<void> {
	...
	private handleSyncRequest(_data: any): void {
		// Handle sync requests from other nodes
	}

	private handleNodeJoined(_data: any): void {
		// Handle new node joining
	}

	private handleNodeLeft(_data: any): void {
		// Handle node leaving
	}

	private handleConflict(_data: any): void {
