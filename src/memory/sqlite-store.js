...
class SqliteMemoryStore {
	...
	async store(key, value, options = {}) {
		...
		try {
			const result = this.statements.get("upsert").run(key, valueStr, namespace, metadata, ttl, expiresAt)
	...
	async search(pattern, options = {}) {
		...
		try {
			const rows = this.statements.get("search").all(namespace, searchPattern, searchPattern, limit)
