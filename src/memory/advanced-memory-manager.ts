...
export class AdvancedMemoryManager extends EventEmitter {
	...
	private updateIndex(entry: MemoryEntry, operation: "create" | "update" | "delete"): void {
		...
		if (operation === "delete") {
			...
			for (const tag of tags) {
				this.removeFromIndex(this.index.tags, tag, id)
			}
			...
			for (const word of words) {
				this.removeFromIndex(this.index.fullText, word, id)
			}
		} else {
			// Add to indices
			this.addToIndex(this.index.keys, key, id)
			for (const tag of tags) {
				this.addToIndex(this.index.tags, tag, id)
			}
			...
			const words = this.extractWords(value)
			for (const word of words) {
				this.addToIndex(this.index.fullText, word, id)
			}
		}
	}
	...
	private queryWithIndex(_options: QueryOptions): MemoryEntry[] {
	...
	private prepareCsvExport(entries: MemoryEntry[], _options: ExportOptions): string {
	...
	private prepareXmlExport(entries: MemoryEntry[], _options: ExportOptions): string {
	...
	private prepareYamlExport(entries: MemoryEntry[], _options: ExportOptions): string {
	...
	private async cleanupOrphanedReferences(_dryRun?: boolean): Promise<number> {
	...
	private async loadPersistedData(): Promise<void> {
		try {
			...
			const _indexFile = join(this.indexPath, "index.json")
	...
	private async encryptData(data: string, _encryption: NonNullable<ExportOptions["encryption"]>): Promise<string> {
